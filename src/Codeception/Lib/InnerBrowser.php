...
class InnerBrowser extends Module implements Web, PageSourceSaver, ElementLocator, ConflictsWithModule
{
    ...
    public function _failed(TestInterface $test, $fail)
    {
        ...
        $filename = codecept_output_dir() . str_replace(['::', '\\', '/'], ['.', '.', '.'], Descriptor::getTestSignature($test));
        $filename = substr($filename, 0, 244) . '.fail.html';
    ...
    protected function getFieldsByLabelOrCss($field)
    {
        ...
        if (count($label)) {
            ...
            if ($label->attr('for')) {
                ...
                $input = $this->strictMatch(['xpath' => sprintf('.//label[*[text()[normalize-space()=%s]]]/input', Crawler::xpathLiteral($field))]);
            }
        }
